         ...
         EvMappingList * mapping_list)
{
	...
	for (l = ev_mapping_list_get_list (mapping_list); l; l = g_list_next (l)) {
        ...
        cairo_rectangle (cr,
            mapping->area.x1, mapping->area.y1,
            mapping->area.x2 - mapping->area.x1, mapping->area.y2 - mapping->area.y1);
        cairo_stroke (cr);
	}
		     ...
		     EvRenderContext *rc)
{
	...
	return pdf_page_render (poppler_page,
				width, height, rc,
                pdf_document_links_get_links (EV_DOCUMENT_LINKS (document), rc->page) // FIXME
                );
}
			 ...
			 gint             height)
{
	...
	surface = pdf_page_render (poppler_page, width, height, rc, NULL);
